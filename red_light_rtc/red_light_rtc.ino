...
void setup() {
  ...
  digitalWrite(BULB, LOW); //RELAY with na (normally open) works with inverse logic, need to be
  //to ground to be connected (Starts ON)
}
...
void loop() {
  ...
  switch (state) {
    ...
    case STANDBY:
      ...
      if (alarm == true) {
        ...
        if (temp_hours == hours && temp_minutes == minutes) {
          ...
          if (bulb_state == LOW) { // RELAY IS ON
            ...
            // Serial.println(bulb_state);
            noInterrupts();
            //digitalWrite(BULB, HIGH); // turn off the bulb, uncomment for turning of the bulb 
            // when ON_TIME passed after the bulb has turned on
            cli();
            interrupts();
            alarm = false;
      ...
      //BUTTON LOGIC FOR CHANGING MODE
      //BUTTON 1
      ...
      // BUTON LOGIC FOR ACTIVATING/DEACTIVATING ALARM
      //BUTTON 2
      if (digitalRead(button2) == LOW) { // first button pressed
        ...
        if ((millis() - buttonTimer2 > longPressTime2) && (longPressActive2 == false)) {
          ...
          // Serial.println("LONG PRESSED BUTTON");
          buttonActive2 = false;
          alarm = !alarm; // change alarm state
          if(alarm == true){
            bulb_state = HIGH;
          }else{
            bulb_state = LOW;
